...
def main():
    ...
    for sku in skus:
        ...
        url_lines.append(
            f"    <loc>{ORIGIN}/product-detail.html?sku={quote(sku, safe='')}</loc>"
        )
        url_lines.append(f"    <lastmod>{LASTMOD}</lastmod>")
        url_lines.append("    <changefreq>weekly</changefreq>")
        url_lines.append("    <priority>0.7</priority>")
        url_lines.append("  </url>")
    prod_xml = (
        '<?xml version="1.0" encoding="UTF-8"?>\n'
        '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n'
    ...
    print(f"Wrote {len(skus)} product URLs, SEO map with {len(seo_map)} SKUs")
